...
class TestTransfer:
    ...
    def test_transfer_between_users(self, api_manager: ApiManager, created_account_factory, amount: float):
        ...
        api_manager.user_steps.transfer(sender, sender_account.id, receiver_account.id, amount)
    ...
    def test_transfer_between_one_users(self, api_manager: ApiManager, created_account_factory):
        ...
        api_manager.user_steps.transfer(sender, account_1.id, account_2.id, 1000)
    ...
    def test_transfer_over_limit(self, api_manager: ApiManager, created_account_factory, amount: float):
        ...
        api_manager.user_steps.transfer_bad_request(sender, sender_account.id, receiver_account.id, amount, "Transfer amount cannot exceed 10000")
    ...
    def test_transfer_below_minimum(self, api_manager: ApiManager, created_account_factory):
        ...
        api_manager.user_steps.transfer_bad_request(sender, sender_account.id, receiver_account.id, -1, "Transfer amount must be at least 0.01")

    def test_transfer_below_balance(self, api_manager: ApiManager, created_account_factory):
        sender, sender_account = created_account_factory(balance=3000)
        receiver, receiver_account = created_account_factory()

        api_manager.user_steps.transfer_bad_request(sender, sender_account.id, receiver_account.id, 3000.01, "Invalid transfer: insufficient funds or invalid accounts")
